...
public class InfoReader {
    ...
    public InfoReader(iShout plugin) {
        this.plugin = plugin;
    }
    ...
    public Object getRawInfo(String name, InfoType type, String world, String info) {
        ...
        if (plugin.useNewInfo)
            return getiShoutInfo(name, type, world, info);
     ...
     * iShout Stuff
     */
    Object getiShoutInfo(String name, InfoType type, String world, String info) {
        if (info.equals("group"))
            return getiShoutGroup(name);

        String iType = type.getName();

        if (plugin.iShIConfig.isSet(iType + "." + name + ".info." + info))
            return plugin.iShIConfig.get(iType + "." + name + ".info." + info);

        else if (plugin.iShIConfig.isSet(iType + "." + name + ".worlds." + world + "." + info))
            return plugin.iShIConfig.get(iType + "." + name + ".worlds." + world + "." + info);

        return "";
    }

    Object getiShoutGroup(String name) {
        if (plugin.iShIConfig.isSet("users." + name + ".group"))
            return plugin.iShIConfig.get("users." + name + ".group");
    ...
    Object getLeveledInfo(String name, String world, String info) {
        ...
        if (!plugin.iShIConfig.isSet("iShout." + info))
            return "";

        if (!plugin.iShIConfig.isSet("rank." + info))
            return getBukkitInfo(name, world, info);

        for (Map.Entry<String, Object> entry : plugin.iShIConfig.getValues(true).entrySet()) {
            if (entry.getKey().contains("iShout." + info + "."))
                if (plugin.getAPI().checkPermissions(name, world, entry.getKey())) {
                    String rVal = entry.getKey().replaceFirst("iShout\\.", "rank.");

                    if (!plugin.iShIConfig.isSet(rVal))
                        continue;

                    try {
                        iMap.put(plugin.iShIConfig.getInt(rVal), entry.getValue().toString());
    ...
    String getPermBukkitGroup(String name) {
        ...
        List<com.platymuus.bukkit.permissions.Group> pGroups = pBukkit.getGroups(name);
    ...
    public String getGroupName(String group) {
        ...
        if (plugin.iShIConfig.isSet("groupnames." + group))
            return plugin.iShIConfig.getString("groupnames." + group);
    ...
    public String getWorldName(String world) {
        ...
        if (plugin.iShIConfig.isSet("worldnames." + world))
            return plugin.iShIConfig.getString("worldnames." + world);
     ...
     * @return Player Name's iShout Alias.
     */
    public String getmName(String name) {
        if (plugin.iShIConfig.isSet("mname." + name))
            if (!(plugin.iShIConfig.getString("mname." + name).isEmpty()))
                return plugin.iShIConfig.getString("mname." + name);
