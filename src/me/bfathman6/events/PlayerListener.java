...
import me.bfathman6.api.EventType;
...
public class PlayerListener implements Runnable, Listener {
    ...
    public PlayerListener(iShout plugin) {
        this.plugin = plugin;
    }
    ...
    public void onPlayerCommandPreprocess(PlayerCommandPreprocessEvent event) {
        ...
        for (String aliases : plugin.getCommand("iShoutafk").getAliases())
            if (event.getMessage().contains("/" + aliases) ||
                    event.getMessage().contains("/iShoutafk"))
                return;
    ...
    public void onPlayerChat(PlayerChatEvent event) {
        ...
        // Message Prefix
        if (plugin.iShPrefix.get(pName) != null
                && !plugin.iShPrefix.get(pName).isEmpty())
            msg = plugin.iShPrefix.get(pName) + msg;
    ...
    public void onPlayerJoin(PlayerJoinEvent event) {
        ...
        if (plugin.useAddDefault)
            if (plugin.iShIConfig.get("users." + pName) == null)
                plugin.getWriter().addBase(pName, plugin.iShIDefaultGroup);
        ...
        if (plugin.alterEvents)
            if (plugin.sJoinB) {
                suppressEventMessage(plugin.getAPI().ParseEventName(mPName, world) + " " + plugin.getReader().getEventMessage(EventType.JOIN), "iShout.suppress.join", "iShout.bypass.suppress.join", plugin.sJoinI);
    ...
    public void onPlayerKick(PlayerKickEvent event) {
        ...
        if (plugin.sKickB) {
            suppressEventMessage(plugin.getAPI().ParseEventName(pName, world) + " " + kickMsg, "iShout.suppress.kick", "iShout.bypass.suppress.kick",plugin.sKickI);
    ...
    public void onPlayerQuit(PlayerQuitEvent event) {
        ...
        if (plugin.sQuitB) {
            suppressEventMessage(plugin.getAPI().ParseEventName(pName, world) + " " + plugin.getReader().getEventMessage(EventType.QUIT), "iShout.suppress.quit", "iShout.bypass.suppress.quit", plugin.sQuitI);
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (block.getTypeId() == 63) {
            ...
            if (sign.getLine(0).equals("[iShout]"))
    ...
    Boolean checkSpy(String player, String world) {
        if (plugin.getAPI().checkPermissions(player, world, "iShout.spy")) {
