...
public class EntityListener implements Listener {
    ...
    public EntityListener(iShout plugin) {
        this.plugin = plugin;
    }
    ...
    void suppressDeathMessage(String pName, String pCause, String world, String dMsg, Integer max, Boolean isPlayer) {
        for (Player player : plugin.getServer().getOnlinePlayers()) {
            if (plugin.getAPI().checkPermissions(player.getName(), player.getWorld().getName(), "iShout.bypass.suppress.death")) {
                player.sendMessage(handlePlayerDeath(pName, pCause, world, dMsg, isPlayer));
                continue;
            }

            if (!(plugin.getServer().getOnlinePlayers().length > max))
                if (!plugin.getAPI().checkPermissions(player.getName(), player.getWorld().getName(), "iShout.suppress.death"))
                    player.sendMessage(handlePlayerDeath(pName, pCause, world, dMsg, isPlayer));
        }
