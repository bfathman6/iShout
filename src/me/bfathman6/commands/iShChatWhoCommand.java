...
public class iShChatWhoCommand implements CommandExecutor {
    iShout plugin;

    public iShChatWhoCommand(iShout plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (cmd.equalsIgnoreCase("iShoutwho")) {
            if (args.length > 0) {
                if (sender instanceof Player) {
                    Player player = (Player) sender;
                    if (!plugin.getAPI().checkPermissions(player.getName(), player.getWorld().getName(), "ishout.who")) {
