...
public class iShChatListCommand implements CommandExecutor {
    iShout plugin;

    public iShChatListCommand(iShout plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (cmd.equalsIgnoreCase("iShoutlist")) {
            if (sender instanceof Player) {
                Player player = (Player) sender;
                if (!plugin.getAPI().checkPermissions(player.getName(), player.getWorld().getName(), "iShout.list")) {
